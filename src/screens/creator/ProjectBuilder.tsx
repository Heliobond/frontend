...
export function ProjectBuilder() {
  ...
  const goalLabel = goalNumber > 0 ? `$0 of $${goalNumber.toLocaleString('en-US')}` : 'awaiting funding'

  return (
    <div style={{ display: 'grid', gap: 24, gridTemplateColumns: 'minmax(0, 1.2fr) minmax(0, 0.8fr)', alignItems: 'start' }}>
      {/* Left — the form */}
      <Card>
        <h3 style={cardTitle}>Project details</h3>
        <p style={{ ...subtle, margin: '0 0 20px' }}>This metadata lives off-chain and links to your on-chain registry entry.</p>

        <Field label="Project name" htmlFor="hb-name">
          <input id="hb-name" type="text" value={name} onChange={(e) => setName(e.target.value)} style={inputStyle} />
        </Field>

        <Field label="Location" htmlFor="hb-bloc">
          <input id="hb-bloc" type="text" value={location} onChange={(e) => setLocation(e.target.value)} style={inputStyle} />
        </Field>
        ...
        <Field label="Funding goal" htmlFor="hb-goal">
          <div style={{ position: 'relative' }}>
            <span style={{ position: 'absolute', left: 14, top: '50%', transform: 'translateY(-50%)', fontFamily: 'var(--font-data)', fontSize: 15, color: 'var(--ink-60)' }}>
              $
            </span>
            <input
              id="hb-goal"
              ...
              style={{ ...inputStyle, paddingLeft: 28, fontFamily: 'var(--font-data)', fontFeatureSettings: '"tnum" 1' }}
            />
          </div>
        </Field>
      ...
      <div style={{ position: 'sticky', top: 24, display: 'flex', flexDirection: 'column', gap: 12 }}>
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}>
          <span aria-hidden="true" style={{ width: 8, height: 8, borderRadius: '50%', background: 'var(--solar)', border: '1px solid var(--ink)' }} />
          <span style={{ fontFamily: 'var(--font-body)', fontSize: 12.5, fontWeight: 600, color: 'var(--ink)' }}>Live preview</span>
          <span style={{ fontFamily: 'var(--font-body)', fontSize: 12.5, color: 'var(--ink-60)' }}>— updates as you type</span>
        </div>
        ...
        <p style={{ ...subtle, margin: 0 }}>
          Credit and green scores show as pending until the oracle verifies your project. The {type.toLowerCase()} type and your story
          travel with this card across the atlas.
        </p>
      </div>
    </div>
  )
...
function DropZone({ label, hint }: { label: string; hint: string }) {
  return (
    ...
    >
      <svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="var(--ink)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
        <path d="M12 16V4" />
        <path d="m7 9 5-5 5 5" />
        <path d="M5 20h14" />
      </svg>
      <div style={{ fontFamily: 'var(--font-body)', fontSize: 13, fontWeight: 600, color: 'var(--ink)' }}>{label}</div>
      <div style={{ fontFamily: 'var(--font-body)', fontSize: 11.5, color: 'var(--ink-60)', lineHeight: 1.4 }}>{hint}</div>
    </div>
  )
}
